...
export const convertToString: EnvironmentVariableConverter<string> = (value) =>
  value;

export const convertToNumber: EnvironmentVariableConverter<number> = (
  value
) => {
  const ret = +value;
...
export function getEnvironmentVariable<T>(
  name: string,
  converter: EnvironmentVariableConverter<T>
): T;
